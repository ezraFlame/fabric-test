...
import com.scarzehd.fabrictest.util.ModTags;
...
public class DowsingRodItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if (context.getWorld().isClient()) {
            ...
            if (!foundBlock) {
                player.sendMessage(new TranslatableText("item.fabrictest.dowsing_rod.no_valuables"), true);
            }
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (Screen.hasShiftDown()) {
            tooltip.add(new TranslatableText("item.fabrictest.dowsing_rod.tooltip.shift"));
        } else {
            tooltip.add(new TranslatableText("item.fabrictest.dowsing_rod.tooltip"));
        }
    }
